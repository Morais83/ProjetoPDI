...
export default function AdminMarcas() {
  ...
  const eliminar = async (id) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/marcas/${id}`, { method: 'DELETE' });
      carregarMarcas();
    } catch (err) {
      console.error(err);
    }
  ...
  const guardar = async () => {
    ...
    try {
      if (marcaEditando) {
        await fetch(`http://localhost:5000/api/marcas/${marcaEditando}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(form),
        });
      } else {
        await fetch('http://localhost:5000/api/marcas', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(form),
        });
